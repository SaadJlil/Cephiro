...
public sealed class ListingSearchRequestValidator: AbstractValidator<ListingSearchRequest>
{
    public ListingSearchRequestValidator()
    {
        ...
        RuleFor(x => x.MaximumPrice).GreaterThan(0).WithMessage("MaximumPrice must be positive.")
            .When(x => x.MaximumPrice is not null);
        ...
        RuleFor(x => x.SurfaceMin).GreaterThanOrEqualTo(0).WithMessage("Min Surface must be positive.")
            .When(x => x.SurfaceMin is not null);
        RuleFor(x => x.SurfaceMax).GreaterThan(0).WithMessage("Max Surface must be positive.")
            .When(x => x.SurfaceMax is not null);
        RuleFor(x => x).Must(x => x.SurfaceMin < x.SurfaceMax)
                       .WithMessage("The maximum surface must be higher than the minimum surface.")
                       .When(x => x.SurfaceMin is not null && x.SurfaceMax is not null);
